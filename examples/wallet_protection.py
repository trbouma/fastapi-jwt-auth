...
@app.post('/login')
def login(wallet_key: Wallet, Authorize: AuthJWT = Depends()):
    """
    ...
    """
    if wallet_key.wallet_key != "trbouma" :
        raise HTTPException(status_code=401,detail="Bad wallet_key")

    # Create the tokens and passing to set_access_cookies or set_refresh_cookies
    access_token = Authorize.create_access_token(subject=wallet_key.wallet_key)
    refresh_token = Authorize.create_refresh_token(wallet_key.wallet_key)
...
def protected(Authorize: AuthJWT = Depends()):
    ...
    current_wallet = Authorize.get_jwt_subject()
    return {"current_wallet": current_wallet}
